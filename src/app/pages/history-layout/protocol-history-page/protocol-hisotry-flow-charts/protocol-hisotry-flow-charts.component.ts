...
@Component({
  selector: 'app-protocol-hisotry-flow-charts',
  standalone: true,
  imports: [],
  templateUrl: './protocol-hisotry-flow-charts.component.html',
  styleUrl: './protocol-hisotry-flow-charts.component.scss',
})
export class ProtocolHisotryFlowChartsComponent {
  ...
  getHistory() {
    this._HistoryProtocolsService.History.subscribe((data) => {
      if (!this.isHistoryBack) {
        if (data?.Historys?.length || 0 > 0) {
          let allQuestions = data?.Historys.map((q) => q.question);
          this.currentQuestionArr = allQuestions as Question[];
          ...
          //   }
          // });
          console.log(this.currentQuestionArr.map((Q) => Q.id));
          let idArr = this.currentQuestionArr.map((Q) => Q.id);
          console.log(idArr, '================================');
          idArr
            .sort((a, b) => a - b)
            .forEach((n, i) => {
